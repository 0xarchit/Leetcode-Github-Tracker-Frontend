...
const tableMetricLookup = tableMetrics.reduce((acc, metric) => {
...
const StatsPage = () => {
    ...
    const loadClasses = async () => {
      ...
      try {
        ...
        setSelectedClass((prev) => {
          if (prev && (prev === ALL_CLASSES_KEY || tables.includes(prev)))
            return prev;
  ...
  const enrichedStudents = useMemo(
    () => computeStudentMetricsWithClass(students),
    [students]
  );
  ...
  const metricAccessor: Record<
    SortMetric,
    (student: StudentMetrics) => number
  > = useMemo(
    () => ({
  ...
  const topStudents = useMemo(
    () => sortedStudents.slice(0, effectiveTopN),
    [sortedStudents, effectiveTopN]
  );
  ...
  const classOverview = useMemo(() => {
    ...
    const activeStudents = enrichedStudents.filter(
      (s) => s.contributions30 > 0 || s.submissions30 > 0
    ).length;
    const studentsWithStreaks = enrichedStudents.filter(
      (s) => s.currentStreak > 0
    ).length;
    const avgAcceptanceRate =
      enrichedStudents
        .filter((s) => s.totalSubmissions > 0)
        .reduce((sum, s) => sum + s.acceptanceRate, 0) /
      Math.max(
        enrichedStudents.filter((s) => s.totalSubmissions > 0).length,
        1
      );

    const topPerformer = enrichedStudents.reduce(
      (max, student) =>
        student.combinedScore > max.combinedScore ? student : max,
      enrichedStudents[0]
    );

    const highPerformers = enrichedStudents.filter(
      (s) => s.combinedScore >= 30
    ).length;

    return {
      activeStudents,
      totalStudents: enrichedStudents.length,
      activityRate: ((activeStudents / enrichedStudents.length) * 100).toFixed(
        1
      ),
      studentsWithStreaks,
      streakRate: (
        (studentsWithStreaks / enrichedStudents.length) *
        100
      ).toFixed(1),
      ...
      highPerformers,
      highPerformerRate: (
        (highPerformers / enrichedStudents.length) *
        100
      ).toFixed(1),
    };
  }, [enrichedStudents]);
  ...
  const radarData = useMemo(() => {
    ...
    const maxPerKey = radarMetrics.reduce<Record<RadarMetricKey, number>>(
      (acc, metric) => {
      ...
      topStudents.forEach((student) => {
        ...
        const numericValue =
          typeof rawValue === "number" && Number.isFinite(rawValue)
            ...
            : 0;
        entry[key] =
          max === 0 ? 0 : Number(((numericValue / max) * 100).toFixed(1));
      });
      return entry;
    });
  }, [topStudents]);
          ...
          <Card>
            <CardHeader className="pb-4">
              <div className="flex flex-col gap-2">
                ...
                <CardDescription>
                  Compare top {effectiveTopN} students{" "}
                  {selectedClass === ALL_CLASSES_KEY ? "from all classes" : ""}{" "}
                  over the last {HISTORY_WINDOW_DAYS} days across GitHub and
                  LeetCode metrics.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={noClasses ? "No classes" : "Select class"}
                      />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <ToggleGroup
                  ...
                  value={sortMetric}
                  onValueChange={(value) =>
                    value && setSortMetric(value as SortMetric)
                  }
          ...
          {noClasses && (
            <Card className="border-dashed border-muted-foreground/50 bg-transparent">
              <CardHeader>
                ...
                <CardDescription>
                  Connect your data source in the dashboard settings to start
          ...
          {!noClasses && !isLoadingStudents && classOverview && (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {classOverview.activityRate}%
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {classOverview.activeStudents} of{" "}
                    {classOverview.totalStudents} active
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {classOverview.streakRate}%
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {classOverview.studentsWithStreaks} students with active
                    streaks
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {classOverview.highPerformerRate}%
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {classOverview.highPerformers} students with score ≥30
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          {!noClasses && (
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>
                  Side-by-side comparison across 16 key indicators. Values are
                  scoped to the last {HISTORY_WINDOW_DAYS}
              ...
              <CardContent>
                {isLoadingStudents ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    No students matched your criteria. Try expanding the top-N
                    range or clearing the search filter.
                  </div>
                ) : (
                  <>
                        ...
                        <table className="w-full caption-bottom text-sm min-w-full">
                          <thead className="sticky top-0 z-20 bg-background border-b border-table-border shadow-sm">
                            <tr className="bg-background hover:bg-background border-b">
                              <th
                                className="h-12 px-4 min-w-[4rem] text-left text-xs font-semibold uppercase tracking-wide sticky left-0 z-30 border-r border-table-border shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]"
                                style={{
                              ...
                              <th
                                className="h-12 px-4 min-w-[9rem] text-left text-xs font-semibold uppercase tracking-wide sticky left-16 z-30 border-r border-table-border shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]"
                                style={{
                                  backgroundColor: "hsl(var(--background))",
                                }}
                              >
                            ...
                            {topStudents.map((student, index) => {
                              ...
                              return (
                                <tr
                                  ...
                                  className="text-xs border-b transition-colors hover:bg-muted/50"
                                >
                                  <td
                                    className="p-4 align-middle sticky left-0 z-10 border-r border-table-border shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]"
                                  ...
                                  <td
                                    className="p-4 align-middle font-medium sticky left-16 z-10 border-r border-table-border shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]"
                                    style={{ backgroundColor: bgColor }}
                                  >
                                  ...
                                  <td className="p-4 align-middle text-sm font-medium">
                                    {student.base.roll_number}
                                  </td>
                                  {tableMetrics.map((metric) => (
                                    <td
                                      key={metric.key}
                                      className="p-4 align-middle whitespace-nowrap text-right text-xs font-medium tabular-nums"
                                    >
                                      {formatTableValue(
                                        student,
                                        metric.key,
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-muted-foreground">
                                Roll: {student.base.roll_number}
                              </p>
                              {selectedClass === ALL_CLASSES_KEY &&
                                student.base.section && (
                                  <p className="text-xs text-muted-foreground">
                                    Section:{" "}
                                    <Badge
                                      variant="outline"
                          ...
                          <div className="grid grid-cols-3 gap-2 mb-3 pb-3 border-b border-border/50">
                            <div className="text-center bg-primary/10 rounded-md p-2">
                              <div className="text-lg font-bold text-primary">
                                {student.combinedScore.toFixed(0)}
                              </div>
                            ...
                            <div className="text-center bg-orange-500/10 rounded-md p-2">
                              <div className="text-lg font-bold text-orange-600 dark:text-orange-400">
                                {student.leetcodeScore.toFixed(0)}
                              </div>
                              <div className="text-[0.65rem] text-muted-foreground">
                                LeetCode
                              </div>
              ...
              <div className="grid gap-6 xl:grid-cols-2">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Stacked view of Easy, Medium, and Hard problems solved by
                      top students.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              <div className="grid gap-6 xl:grid-cols-2">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Total submissions vs. solved problems with acceptance
                      rate.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    ...
                    >
                      <BarChart data={submissionInsightsData}>
                        ...
                        <Bar
                          dataKey="totalSolved"
                          fill="var(--color-totalSolved)"
                          radius={[6, 6, 0, 0]}
                        />
                      </BarChart>
                    </ChartContainer>
                    <div className="mt-4 grid grid-cols-3 gap-2 text-center text-sm">
                      {submissionInsightsData
                        .slice(0, 3)
                        .map((student, idx) => (
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Radar chart normalised per metric for the top{" "}
                      {effectiveTopN} students.
                    </CardDescription>
                  </CardHeader>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-xs font-medium whitespace-nowrap">
                              {radarConfig[dataKey]?.label ??
                                student.base.name.split(" ")[0]}
                            </span>
                          </div>
                        );
                      })}
              ...
              <div className="grid gap-6 xl:grid-cols-2">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Contributions, followers, and public repos in the past{" "}
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Total submissions (all-time), problems solved, and active
                      streaks.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
